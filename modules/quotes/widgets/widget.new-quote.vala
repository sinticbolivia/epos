...
namespace EPos
{
	public class WidgetNewQuote : Gtk.Box
	{
		...
		protected 	Box				boxNewQuote;
		protected	Image			image1;
		protected 	Label			labelTitle;
		protected 	ComboBox		comboboxStores;
		protected	SBDatePicker	datepickerOrderDate;
		protected	SBDatePicker	datepickerExpirationDate;
		protected	Entry			entrySearchCustomer;
		protected	Button			buttonLookupCustomers;
		protected	Button			buttonNewCustomer;
		...
		{
			this.ui = ((SBModules.GetModule("Quotes") as SBGtkModule)).GetGladeUi("new-quote.glade");
			this.expand = true;			
			this.boxNewQuote		= (Box)this.ui.get_object("boxNewQuote");
			...
			this.datepickerExpirationDate	= new SBDatePicker();
			this.datepickerOrderDate.Icon = GtkHelper.GetPixbuf("share/images/calendar-icon-16x16.png");
			this.datepickerExpirationDate.Icon = GtkHelper.GetPixbuf("share/images/calendar-icon-16x16.png");
			this.datepickerOrderDate.show();
			this.datepickerExpirationDate.show();
			
			grid2.attach(this.datepickerOrderDate, 3, 0, 1, 1);
			grid2.attach(this.datepickerExpirationDate, 5, 0, 1, 1);
			
			this.entrySearchCustomer	= (Entry)this.ui.get_object("entrySearchCustomer");
			this.buttonLookupCustomers	= (Button)this.ui.get_object("buttonLookupCustomers");
			this.buttonNewCustomer	= (Button)this.ui.get_object("buttonNewCustomer");
			...
			this.boxNewQuote.reparent(this);
			this.Build();
			this.FillForm();
			this.SetEvents();
		...
		protected void Build()
		{
			...
			this.datepickerExpirationDate.DateString = new DateTime.now_local().format("%Y-%m-%d");
			...
			//##entry search customer completion
			...
			this.entrySearchCustomer.completion.set_match_func( (completion, key, iter) => {return true;});
			...
			string[,] columns = 
			{
				...
				{SBText.__("Price"), "text", "70", "right", "editable"},
				{SBText.__("Sub Total"), "text", "70", "right", ""},
				{SBText.__("Discount"), "text", "70", "right", "editable"},
		...
		{
			this.buttonLookupCustomers.clicked.connect(this.OnButtonLookupCustomersClicked);
			...
			var cell_cost = (CellRendererText)this.treeviewProducts.get_column(Columns.PRICE).get_cells().nth_data(0);
			...
			cell_cost.edited.connect( (path, new_text) => 
			{
				...
				TreeIter iter;
				this.treeviewProducts.model.get_iter(out iter, new TreePath.from_string(path));
				(this.treeviewProducts.model as ListStore).set_value(iter, Columns.PRICE, "%.2f".printf(cost));
				this.calculateRowTotal(iter);
				this.calculateTotals();
			});
		...
		{
			var win = new WindowCustomerLookup(){modal = true};
			
			win.OnCustomerSelected.connect( (data, the_window) => 
			{
				this.customerId = int.parse(data["customer_id"]);
				this.entrySearchCustomer.text = "%s %s".printf((string)data["first_name"], (string)data["last_name"]);
			});
			win.show();
		}
		...
		protected void OnButtonAddProductClicked()
		{
			...
			else
			{
				...
				string? trid = SBProduct.GetMeta(pid, "tax_rate_id");
				...
				*/
				double sub_total	= prod.Price * qty;
		...
		{
			Value v_qty, v_price, v_rate;
			
			this.treeviewProducts.model.get_value(iter, Columns.QTY, out v_qty);
			this.treeviewProducts.model.get_value(iter, Columns.PRICE, out v_price);
			...
			double sub_total	= (int)v_qty * price;
			double tax_rate		= ((int)v_qty * price) * (rate / 100);
		...
		protected void OnButtonSaveClicked()
		{
			...
			{
				Value v_pid, v_name, v_qty, v_price, v_subtotal, v_tax_rate, v_total_tax, v_total;
				
				model.get_value(iter, Columns.QTY, out v_qty);
				model.get_value(iter, Columns.PRICE, out v_price);
				model.get_value(iter, Columns.SUB_TOTAL, out v_subtotal);
				model.get_value(iter, Columns.PRODUCT_ID, out v_pid);
				model.get_value(iter, Columns.PRODUCT_NAME, out v_name);
				...
				item.set("product_name", (string)v_name);
			...
			order.set("description", this.entryDetails.text.strip());
			...
			order.set("quote_date", this.datepickerOrderDate.DateString);
			order.set("expiration_date", this.datepickerExpirationDate.DateString);
			...
			if( this.quote == null )
			{
				...
				order.set("customer_id", this.customerId);
				order.set("creation_date", cdate);
				
				long quote_id = dbh.Insert("quotes", order);
				//##Set purchar order code
				string code = "QT-%s".printf(Utils.FillCeros((int)quote_id, 6));
				var data = new HashMap<string, Value?>();
				data.set("code", code);
				var w = new HashMap<string, Value?>();
				w.set("quote_id", quote_id);
				dbh.Update("quotes", data, w);
				//##insert quote items
				foreach(HashMap<string, Value?> item in items)
				{
					item.set("quote_id", quote_id);
					dbh.Insert("quote_items", item);
				}
				
				var msg = new InfoDialog("success")
				{ 
					Title	= SBText.__("Quote created"),
					Message = SBText.__("The quote has been register.")
				};
				msg.run();
				msg.destroy();
			}
			else
			{
				...
				var w = new HashMap<string, Value?>();
				w.set("quote_id", this.quote.Id);
				dbh.Update("quoted", order, w);
				dbh.Execute("DELETE FROM quote_items WHERE quote_id = %d".printf(this.quote.Id));
				foreach(HashMap<string, Value?> item in items)
				{
					item.set("quote_id", this.quote.Id);
					dbh.Insert("quote_items", item);
				}
				
				var msg = new InfoDialog("success")
				{
					Title = SBText.__("Quote updated."),
					Message = SBText.__("The quote has been updated")
				};
				msg.run();
				msg.destroy();
				//##update order object
				this.quote = new Quote.from_id(this.quote.Id);
			}
			dbh.EndTransaction();
		...
		protected void OnButtonCancelClicked()
		{
			...
			string quote_id = (string)this.get_data<string>("tab_id");
			notebook.RemovePage(quote_id);
		}
		public void SetOrder(Quote order)
		{
			...
			foreach(SBDBRow item in order.Items)
			{
				...
				Gdk.Pixbuf remove_img = (SBModules.GetModule("Quotes") as SBGtkModule).GetPixbuf("remove-20x20.png");//SB_ModuleInventory.GetPixbuf("remove-20x20.png");
				this.total_items++;
			...
			{
				var btn_sale = new Button.with_label(SBText.__("Send to Sale"));
				btn_sale.get_style_context().add_class("button-blue");
				btn_sale.show();
				btn_sale.clicked.connect(this.OnButtonSendSaleClicked);
				this.buttonbox1.pack_start(btn_sale);
			}
